...
folders it needs â€” `shared/` is the one folder both the frontend
...
| Path | What it pins |
|---|---|
| `schemas/` | JSON Schemas â€” the canonical, machine-validatable contracts (events, webhook envelopes) |
...
- **Language-neutral core.** The schemas in `schemas/` are JSON Schema, not
  TS/Python â€” each side derives its own types. `types/` documents the event
...
- **Events over the LiveKit data channel.** Realtime state (transcripts,
  indicators, tool activity, errors) flows agent â†’ browser as typed messages
  on the data channel. Client â†’ agent messages (push-to-talk, config) use the
  same channel with `client.*` types.
- **n8n via HTTP only.** The agent never calls the n8n API â€” only configured
